...
<div>
  ...
  <div class="md-layout">
    ...
    <div class="md-layout-item md-medium-size-33 md-small-size-50 md-xsmall-size-100">
      <div class="md-layout-item md-gutter gallery-container"> <img src="../assets/mosaic-logo.svg" alt="mosaic logo"> </div>
      <div class="md-layout-item md-elevation-4 md-gutter">
        <md-field> <label>Upload your photo ...</label>
          <md-file v-model.lazy="uploadFileName" v-on:change="signalFileChange" accept="image/*" />
        </md-field>
      </div>
      <div class="gallery-label md-layout-item md-gutter md-alignment-center-center"> <label>OR select from a photo gallery ... </label> </div>
      ...
      <div class="gallery-container">
        <md-card class="md-layout-nowrap" md-with-hover v-for="cardIndex in 8" :key="cardIndex">
          <div class="md-card-wrapper" @click="galleryItemClicked(cardIndex-1)"> <template v-if="cardIndex < 8"> <transition name="component-fade" mode="out-in">
                <!-- smooth transition from spinner to image --><template v-if="cardImages[cardIndex-1].src">
                  <!-- when image data is present --><img :src="cardImages[cardIndex-1].src" :key="cardImages[cardIndex-1].src" :alt="'card'+cardIndex"> </template> <template v-else> <div class="card-spinner">
                    <!-- when src is '' then a spinner is shown -->
                    <md-progress-spinner :md-diameter="15" :md-stroke="3" md-mode="indeterminate"></md-progress-spinner>
                  </div> </template> </transition> </template> <template v-else> <md-card-actions>
                <md-button class="md-icon-button">
                  <md-icon>navigate_next</md-icon>
                </md-button>
              </md-card-actions> </template> </div>
        </md-card>
      </div>
    </div>
    <div class="md-layout-item  md-xsmall-hide"></div>
  ...
  <md-dialog :md-active.sync="showDialog">
    ...
    <md-tabs md-dynamic-height>
      <md-tab md-label="Mosaic" style="text-align:center;"> <img :src="mosaicImage" style="widht:250px;height:250px;" /> </md-tab>
      <md-tab md-label="Original" style="text-align:center;"> <img :src="originalImage" style="widht:250px;height:250px;" /> </md-tab>
    </md-tabs>
    <div style="padding-left:20px;padding-top:15px;">{{ imageLoadMessage }} <span v-if="imgurUrl"><md-icon>arrow_right_alt</md-icon></span> <b v-if="imgurUrl">{{imgurUrl}}</b> </div>
    <md-dialog-actions>
...
export default {
  ...
  methods: {
    openGallery (galleryURL, isAuthorized) {
      const c = this
      this.clearGallery()
    ...
    clearGallery () {
      for (let i = 0; i < this.cardImages.length - 1; i++) { // show spinners
        this.cardImages[i].src = ''
      }
    },
    galleryItemClicked (clickedIndex) { // thumbnail slider click handler
      if (clickedIndex === 7) { // next page clicked
        let imgsReady = true
        for (let i = 0; i < this.cardImages.length - 1; i++) {
        ...
        if (imgsReady) {
          this.clearGallery()
          // handle next page
          this.galleryOffset = this.galleryArrayToScreen(this.galleryImagesLinks, this.galleryOffset + 7)
    ...
    imageLoaded (img) {
      ...
      const newimageurl = this.generateImagePair(img)
      if (newimageurl) {
    ...
    galleryURLtoArray (galleryURL, isAuthorized) {
        ...
        this.$http.get(galleryURL, (isAuthorized ? {
          ...
          if (isAuthorized) { // TODO remove - true = imgur
            ...
            for (let i = 0; i < response.data.data.children.length; i++) {
              let fileName = response.data.data.children[i].data.link_url
              if ((fileName.startsWith('https://i.imgur.com/') || fileName.startsWith('https://i.redd.it/')) && (fileName.endsWith('gif') || fileName.endsWith('png') || fileName.endsWith('jpg'))) {
                imageLinks.push(fileName)
              }
            }
          }
    ...
    galleryArrayToScreen (galleryArray, pageOffset) {
      // imageURLtoThumbnail(imageURL)
      const c = this
      let realOffset = pageOffset
      if (realOffset + 7 >= galleryArray.length) {
        realOffset = galleryArray.length - 7
      }
      for (let i = 0; i < 8; i++) {
        const index = realOffset + i
        if (index < galleryArray.length) {
          this.imageURLtoThumbnail(galleryArray[index], i).then((data) => {
    ...
    imageURLtoThumbnail (imageURL, cIndex) {
      const c = this
      return new Promise((resolve, reject) => {
    ...
    generateImagePair (image) {
      ...
      let xpos = config.borderThickness
      let ypos = config.borderThickness
      if (image.width > config.imgWidth - config.borderThickness * 2 || image.height > config.imgHeight - config.borderThickness * 2) { // larger than canvas - OK
      ...
      const dbuf = document.createElement('canvas') // double buffer as mosaic pixels data source
      dbuf.width = newWidth / config.mosaicRes
      dbuf.height = newHeight / config.mosaicRes
      const ctx2 = dbuf.getContext('2d')
      ...
      const realWidth = dbuf.width * config.mosaicRes
      const realHeight = dbuf.height * config.mosaicRes
      // recalculate border offsets for the mosaic
      xpos = (config.imgWidth - realWidth) / 2
      ypos = (config.imgHeight - realHeight) / 2
      const result = this.renderMosaic(ctx2, // mosaic pixels
        ctx, // targetContext
    ...
    renderMosaic (sourceContext, targetContext, w, h, xoffset, yoffset) {
      try {
        const imgd = sourceContext.getImageData(0, 0, w, h)
        const pix = imgd.data
        let cnt = 0
        for (let i = 0, n = pix.length; i < n; i += 4) {
          ...
          targetContext.arc(
            (cnt % w) * config.mosaicRes + xoffset + config.mosaicRes / 2, // compute projected canvas coords
            Math.floor(cnt / w) * config.mosaicRes + yoffset + config.mosaicRes / 2, config.mosaicRes / 2, 0, 2 * Math.PI)
          targetContext.fill()
          cnt++
        }
      } catch (e) {
